---
title: "correlacion"
author: "Judit"
date: "2024-02-27"
output:
  word_document: default
  html_document:
    df_print: paged
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## R Markdown

This is an R Markdown document. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

When you click the **Knit** button a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. You can embed an R code chunk like this:

```{r cars}
summary(cars)
```

## Including Plots

You can also embed plots, for example:

```{r pressure, echo=FALSE}
plot(pressure)
```

Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot.


### Ejercicio 1

```{r, echo = TRUE}
library(readxl)
data <- as.data.frame(read_excel("C:/correlacion/data.xlsx"))
View(data)
print(data)
```

### Ejercicio 2
```{r}
#Funcion para agregar coeficientes de correlacion
panel.cor <- function(x, y, digits = 2, prefix = "", cex.cor, ...) {
  usr <- par("usr")
  on.exit(par(usr))
  par(usr = c(0, 1, 0 ,1))
  Cor <- abs(cor(x, y)) 
  txt <- paste0(prefix, format(c(Cor, 0.123456789), digits = digits)[1])
  if(missing(cex.cor)) {
    cex.cor <- 0.4 / strwidth(txt)
  }
  text(0.5, 0.5, txt,
       cex = 1 + cex.cor*Cor)
}
#Dibujamos la matriz de correlacion
pairs(data,
      upper.panel = panel.cor, # Panel de correlacion
      lower.panel = panel.smooth) #Curvas de regresión suavizadas
```

### Ejercicio 3
```{r, echo = TRUE}
cor.test(data$longitud, data$peso)
```

### Ejercicio 4
```{r, warning = FALSE}
library(correlation)
resultados <- correlation(data)
resultados
```

### Ejercicio 5
```{r, echo = TRUE}
library(ggpubr)
library(ggplot2)
ggscatter(data, x = "longitud", y = "peso", color = "pink",
          add = "reg.line", conf.int = TRUE,
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "longitud piezas (mm)", ylab = "Peso piezas (mg)")
```

```{r }
library(corrplot)
corrplot(cor(data))
```

```{r }
library(corrplot)
corrplot.mixed(cor(data))
```

### Ejercicio 6
A partir de las siguientes variables

- Distancia (km): 1.1,100.2,90.3,5.4,57.5,6.6,34.7,65.8,57.9,86.1
- Número de cuentas (valor absoluto): 110,2,6,98,40,94,31,5,8,10

#a. Crea 2 vectores: "distancia" y "n_piezas"

```{r }
distancia <- c(1.1,100.2,90.3,5.4,57.5,6.6,34.7,65.8,57.9,86.1)
n_piezas <- c(110,2,6,98,40,94,31,5,8,10)
dist_ncuent <- data.frame(distancia,n_piezas)
knitr::kable(dist_ncuent)
```
#b. Calcula el coeficiente de correlación
La correlación es de -0.9249824 

#c. calcula el nivel de significacncia
El nivel de significancia es de 0.0001265

#d. Intervalo de confianza al 95% en relación con el coeficiente de correlación.
El intervalo de confianza al 95% es de [-0.9824414, -0.7072588].

#e. ¿Que intensidad y dirección presentan ambas variables?
Como es un coeficiente negativo, la dirección es inversa entre ambas variables y su intensidad es de 0.9249824, bastante cercana al máximo que es 1.

#f. ¿Es significativa esta relación?
Si, la relación es significativa pues el nivel de significancia es inferior a 0.05 o lo que es lo mismo inferior al 5% de posibilidades de que esta relación sea resultado del azar. 

#g. ¿Resultaría apropiado afirmar la correlación (o no) entre variables con un tamaño muestral tan reducido (n=10)?
Los datos muestran que es una correlación significativa, por lo que cabría experar que aunque aumenten el número de muestras, no debería haber una variación significativa. Como hemos comprobado que la correlación no se debe al azar sino que responde a una correlación lineal, que se debería mantener conforme aumente el número de muestras. Sin embargo, en estadistica para que sea significativo se suelen emplear más muestras, por ejemplo, n=30 o n=100, normalmente.

```{r, echo = TRUE}
cor.test(dist_ncuent$distancia, dist_ncuent$n_piezas)
```


```{r, echo = TRUE}
correlation::correlation(dist_ncuent)
```


